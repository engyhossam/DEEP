...
# Clean terminal from PIL warning (not an error)
...
def _safe_copy(src: Path, dst: Path):
    """Fast on same drive: try hardlink, else copy."""
    dst.parent.mkdir(parents=True, exist_ok=True)
    if dst.exists():
        return
    try:
        os.link(src, dst)  # hard link (fast)
    except Exception:
...
# -----------------------------
# E) Model (pretrained + custom head)
# -----------------------------
